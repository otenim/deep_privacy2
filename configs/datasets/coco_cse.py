...
data = dict(
    ...
    train_evaluation_fn=functools.partial(
        compute_metrics_iteratively, cache_directory=Path(metrics_cache, "coco_cse_val"), include_two_fake=False
    ),
    evaluation_fn=functools.partial(
        final_eval_fn, cache_directory=Path(metrics_cache, "coco_cse_val_final"), include_two_fake=True
    ),
)
