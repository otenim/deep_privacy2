...
class StyleGANAugmentPipe(torch.nn.Module):
    ...
    def forward(self, batch, debug_percentile=None):
        ...
        if G_inv is not I_3:
            ...
            batch["mask"] = torch.nn.functional.pad(
                input=batch["mask"], pad=[mx0, mx1, my0, my1], mode="constant", value=1.0
            )
            batch["E_mask"] = torch.nn.functional.pad(
                input=batch["E_mask"], pad=[mx0, mx1, my0, my1], mode="constant", value=0.0
            )
            batch["vertices"] = torch.nn.functional.pad(
                input=batch["vertices"], pad=[mx0, mx1, my0, my1], mode="constant", value=0.0
            )
            ...
            batch["mask"] = torch.nn.functional.interpolate(
                batch["mask"][:, :, Hz_pad * 2 : -Hz_pad * 2, Hz_pad * 2 : -Hz_pad * 2],
                scale_factor=0.5,
                mode="nearest",
                recompute_scale_factor=False,
            )
            batch["E_mask"] = torch.nn.functional.interpolate(
                batch["E_mask"][:, :, Hz_pad * 2 : -Hz_pad * 2, Hz_pad * 2 : -Hz_pad * 2],
                scale_factor=0.5,
                mode="nearest",
                recompute_scale_factor=False,
            )
            batch["vertices"] = torch.nn.functional.interpolate(
                batch["vertices"][:, :, Hz_pad * 2 : -Hz_pad * 2, Hz_pad * 2 : -Hz_pad * 2],
                scale_factor=0.5,
                mode="nearest",
                recompute_scale_factor=False,
            )
