...
def binary_dilation(im: torch.Tensor, kernel: torch.Tensor):
    ...
    im = torch.nn.functional.conv2d(im, kernel, groups=im.shape[1], padding=padding)
    im = im > 0.5
    return im
...
def binary_erosion(im: torch.Tensor, kernel: torch.Tensor):
    ...
    im = torch.nn.functional.conv2d(im, kernel, groups=im.shape[1])
...
def remove_pad(x: torch.Tensor, bbox_XYXY, imshape):
    ...
    padding = [max(0, -x0), max(0, -y0), max(x1 - W, 0), max(y1 - H, 0)]
    x0, y0 = padding[:2]
